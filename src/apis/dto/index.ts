...
import { GetCategoryCommendBoardListResponseDto, GetNewBoardListResponseDto, GetRankingBoardListResponseDto, GetRecipeListResponseDto } from "./response/board";
...
const GET_RANKING_BOARD_LIST_URL = (selected: string, times: string) => `${API_DOMAIN}/board/ranking/board-list/${selected}/${times}`;
// description: get recipe list API end point //
const GET_RECIPE_LIST_URL = (searchWord: string, type: string, way: string, material: string) => `${API_DOMAIN}/board/recipe-list/${searchWord}/${type}/${way}/${material}`;
const GET_NO_WORD_RECIPE_LIST_URL = (type: string, way: string, material: string) => `${API_DOMAIN}/board/recipe-list/${type}/${way}/${material}`;

// description: get ranking board list request //
export const getRankingListRequest = async (selected: string, times: string) => {
    const result = axios.get(GET_RANKING_BOARD_LIST_URL(selected, times))
        .then(response => {
...
export const getRecipeListRequest = async (searchWord: string, type: string, way: string, material: string) => {
  const result = axios.get(searchWord ? GET_RECIPE_LIST_URL(searchWord, type, way, material) : GET_NO_WORD_RECIPE_LIST_URL(type, way, material))
    .then(response => {
